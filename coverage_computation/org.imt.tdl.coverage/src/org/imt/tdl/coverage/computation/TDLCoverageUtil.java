...
public class TDLCoverageUtil {
	...
	public void checkInheritanceForNotCoverableClasses() {
		for (EClassifier clazz : metamodelRootElement.getEClassifiers()) {
			...
			if (clazz instanceof EClass) {
				if (!coverableClasses.contains(className) && !extendedClassesWithoutStep.contains(className)) {
	...
	private void checkDynamicAspectsOfClass(EClassifier clazz) {
		List<EAnnotation> classDynamicAnnotations = clazz.getEAnnotations().stream()
				.filter(a -> a.getSource().equals("dynamic") || a.getSource().equals("aspect"))
				.collect(Collectors.toList());
		// if the type of the object is dynamic, all of its features must be set to the
		// default values
	...
	private void findDSLSpecificCoverage() {
		...
		// 1. if the rules are implemented in a java class, retrieve them using
		// extension point
		if (dslSpecificCoverageExtension != null && dslSpecificCoverageExtension.getDomainSpecificCoverage() != null) {
		...
		// 2. else, check .dsl file for the path to the coverageRules.cov file
			...
			if (coverageFilesPathes != null) {
				...
				coverageFilesPathes
						.forEach(path -> resSet.getResource(URI.createPlatformPluginURI(path.trim(), false), true));
				EcoreUtil.resolveAll(resSet);
				resSet.getResources()
						.forEach(r -> dslSpecificCoverages.add((DomainSpecificCoverage) r.getContents().get(0)));
			}
		}
	}
