...
public class ALEEngineLauncher extends AbstractEngine {
	...
	private AleEngine createExecutionEngine() throws EngineContextException, CoreException {
		// if the resource is updated (e.g., the value of its dynamic features are set
		// by the test case)
	...
	protected String getModelEntryPointMethodName() {
		...
		List<Method> mainOperations = parsedSemantics.stream().filter(sem -> sem.getRoot() != null)
				...
				.flatMap(xtdCls -> xtdCls.getMethods().stream()).filter(op -> op.getTags().contains("main"))
				...
				try (IAleEnvironment environment = Helper.gemocDslToAleDsl(language)) {
					...
					try {
						...
						initOperation = parsedSemantics.stream().filter(sem -> sem.getRoot() != null)
								...
								.filter(op -> op.getTags().contains("init")).findFirst();
					} catch (Exception e) {
					}
	...
	public Trace<Step<?>, TracedObject<?>, State<?, ?>> getExecutionTrace() {
		return aleEngine != null
				? (Trace<Step<?>, TracedObject<?>, State<?, ?>>) aleEngine.getAddon(GenericTraceEngineAddon.class)
						.getTrace()
				: null;
	}
