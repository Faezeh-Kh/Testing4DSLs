...
public class DSLProcessor {
	...
	public Path getDSLPath(String DSLName) {
		language = getDSLFile(DSLName);
		if (language != null) {
	...
	private IConfigurationElement getDSLFile(String DSLName) {
		IConfigurationElement language = Arrays
				.asList(Platform.getExtensionRegistry().getConfigurationElementsFor(xdsml_extension_point_id)).stream()
				.filter(l -> l.getAttribute("xdsmlFilePath").endsWith(".dsl") && l.getAttribute(NAME).equals(DSLName))
	...
	public void findDSLExtendedClasses() {
		if (language == null) {
			language = getDSLFile(getDSLEntryValue(NAME));
		}
	...
	private void findK3Classes() {
		...
		final List<Class<?>> classes = dsl.getEntries().stream().filter(e -> e.getKey().equals(K3)).findFirst()
	...
	public String getDSLName() {
		String[] dslFullName = getDSLID().split("\\.");
		return dslFullName[dslFullName.length - 1];
	}

	// NOTE: currently, we only consider K3 and ALE metaprogramming approaches for
	// the implementation of the DSL's semantics
	...
	public EPackage getMetamodelRootElement() {
		if (metamodelRootElement == null && dslEntries.get(ECORE) != null) {
			Resource metamodelRes = (new ResourceSetImpl()).getResource(getEcorePluginURI(), true);
	...
	public BehavioralInterface getBehavioralInterfaceRootElement() {
		if (interfaceRootElement == null) {
			Resource interfaceRes = (new ResourceSetImpl()).getResource(getBehavioralInterfacePluginURI(), true);
			interfaceRootElement = (BehavioralInterface) interfaceRes.getContents().get(0);
		}
